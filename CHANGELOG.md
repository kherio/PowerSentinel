...
### v3.7.1
  - App picker: apps you've marked as allowed or restricted now pin to the top of the list (alphabetically sorted among themselves, with a subtle divider), instead of staying in their normal alphabetical position - no more scrolling through the whole list to see which ones are currently selected. Verified the sorting logic against several scenarios (nothing selected, one selected, two selected across both lists, deselecting, and with a search filter active).
...
### v3.7.0
  ...
  - Fixed a design bug caught while building this: both new backends briefly defined their own identically-named-but-distinct `PowerSentinelApiError` class, which would have silently broken any future `instanceof` check depending on which backend happened to be active - consolidated into one shared class (`errors.js`).
